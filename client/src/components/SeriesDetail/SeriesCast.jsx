      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-3 sm:gap-4">
        {cast &&
          cast.map((actor, index) => (
            <div
              key={index}
              className="aspect-square bg-brand-surface border border-brand-border rounded-lg hover:bg-brand-background hover:border-brand-primary hover:scale-105 hover:shadow-[0_8px_24px_rgba(79,195,247,0.3)] transition cursor-pointer overflow-hidden"
            >
              {actor.image ? (
                <img
                  src={actor.image}
                ...
                <div className="w-full h-full flex items-center justify-center">
                  <span className="text-grey-60 text-center text-xs">
                    {actor.name}
                  </span>
                </div>
              )}
